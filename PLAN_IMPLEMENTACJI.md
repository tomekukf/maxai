...
**Krok 11.3 — Onboarding katalogu PDF: instrukcja w GUI (bez wyzwalania z GUI)**
- **Decyzja:** rezygnujemy z uruchamiania analizy PDF z przeglądarki (webowe GUI nie odpali lokalnego Pythona/LLM).
  Zamiast tego: **instrukcja w panelu admina** (zakładka Dokumentacja) opisująca cały przepływ, **wiedza w kontekście
  Claude** (żeby wiedział, jak to zrobić i wypluć rawdata) oraz **info do ręcznej orchestracji** (komendy krok po kroku).
- Przepływ (lokalnie, 0 Bedrock vision): `prepare-catalog.py <pdf> <nazwa>` → `CLAUDE_INSTRUCTIONS.md` → „Claude, przygotuj
